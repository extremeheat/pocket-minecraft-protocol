module.exports = native => {
  const ctx = {
    ...
    TransactionActions: (buffer, offset) => {
      let { value: network_ids, size: network_idsSize } = (ctx.bool)(buffer, offset)
      let { value: actions, size: actionsSize } = ((buffer, offset) => {
        ...
        for (let i = 0; i < count; i++) {
          ...
          })(buffer, offset + source_typeSize + inventory_idSize + actionSize + flagsSize + slotSize + old_itemSize + new_itemSize)
          return { value: { source_type, inventory_id, action, flags, slot, old_item, new_item, new_item_stack_id }, size: source_typeSize + inventory_idSize + actionSize + flagsSize + slotSize + old_itemSize + new_itemSize + new_item_stack_idSize}
        })(buffer, offset + size)
          data.push(elem.value)
          size += elem.size
        }
        return { value: data, size }
      })(buffer, offset + network_idsSize)
      return { value: { network_ids, actions }, size: network_idsSize + actionsSize}
    },
    ...
    ItemStack: (buffer, offset) => {
      let { value: stack_id, size: stack_idSize } = (ctx.varint)(buffer, offset)
      let { value: item, size: itemSize } = (ctx.Item)(buffer, offset + stack_idSize)
      return { value: { stack_id, item }, size: stack_idSize + itemSize}
    },
    ...
    Skin: (buffer, offset) => {
      ...
      let { value: premium, size: premiumSize } = (ctx.string)(buffer, offset + skin_idSize + play_fab_idSize + skin_resource_packSize + skin_dataSize + animationsSize + cape_dataSize + geometry_dataSize + animation_dataSize)
    ...
    packet_respawn: (buffer, offset) => {
      ...
      let { value: runtime_entity_id, size: runtime_entity_idSize } = (ctx.varint64)(buffer, offset + xSize + ySize + zSize + stateSize)
